...
export default function PlayerClient({ venueId, email }: { venueId: string; email: string }) {
  ...
  // KTV：換歌時先抓「影片內建字幕(CC)」文字（我們自己渲染成大字），沒有才退回 LRCLIB 歌詞
  useEffect(() => {
    if (mode !== 'ktv' || !now) {
      setLyrics([]);
      setLyricStatus('idle');
    ...
    (async () => {
      // 1) 影片字幕（時間軸與影片完全同步）
      try {
        const cc = await fetch('/api/captions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ videoId: song.video_id }),
        }).then((x) => x.json());
        if (cancelled) return;
        if (cc.success && Array.isArray(cc.lines) && cc.lines.length) {
          setLyrics(cc.lines as LyricLine[]);
          setLyricMatch('🎬 影片字幕');
          setLyricStatus('synced');
          return;
        }
      } catch {
        /* 影片沒字幕或抓取失敗，往下退回 LRCLIB */
      }
      // 2) LRCLIB 歌詞庫
      try {
    ...
    <main className="flex min-h-screen flex-col lg:flex-row">
      ...
      <section className="relative flex flex-1 flex-col items-center justify-center p-6">
        ...
        <div className="relative aspect-video w-full max-w-3xl overflow-hidden rounded-2xl border border-[var(--border-strong)] bg-black shadow-[0_20px_60px_-20px_rgba(0,0,0,0.8)]">
          ...
          {/* KTV：歌詞（影片字幕 或 LRCLIB）疊在變暗的影片上，統一大字呈現 */}
          {mode === 'ktv' && (
